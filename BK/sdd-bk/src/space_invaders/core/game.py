...
from space_invaders.core.constants import ACTIONS, BACKGROUND_ASSET_IDS, DEFAULT_BACKGROUND_ID, SceneName
...
class Game:
    def __init__(self) -> None:
        ...
        self.asset_manager = AssetManager(
            ...
            background_ids=BACKGROUND_ASSET_IDS,
    ...
    def _load_persisted_settings(self) -> None:
        saved_settings = self.settings_repository.all()
        if "master_volume" in saved_settings:
            try:
                self.settings_service.update_volume(float(saved_settings["master_volume"]))
            except ValueError:
                pass
        if "debug_mode" in saved_settings:
            self.settings_service.value.debug_mode = saved_settings["debug_mode"] == "1"
        if "background_id" in saved_settings and saved_settings["background_id"] in BACKGROUND_ASSET_IDS:
            self.settings_service.set_background(saved_settings["background_id"])
